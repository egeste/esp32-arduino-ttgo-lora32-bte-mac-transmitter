...
void sniffer(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (type == 0) { // WIFI_PKT_CTRL 
    ...
    for (int i=8;i<=p->rx_ctrl.sig_len;i++) packet += String(p->payload[i],HEX);
    for (int i=4;i<=15;i++) mac += packet[i];

    for (int i=0;i<mac.length();i=i+2) {
      if ((i+2)<mac.length()) mac_eui+=mac.substring(i, i+2)+":";
      if ((i+2)==mac.length()) mac_eui+=mac.substring(i, i+2);
    }
...
void loop_wifi() {
  ...
  if (curChannel > WIFI_MAX_CH) curChannel = 1;
  esp_wifi_set_channel(curChannel, WIFI_SECOND_CHAN_NONE);
  ...
  for (int i=0; i < listcount; i++) {
    print("mac|" + maclist[i][0] + String("(") + maclist[i][3] + String("), "));
  }
}